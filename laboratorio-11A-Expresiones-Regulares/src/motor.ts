...
export const esFormatoIBANValido = (iban: string): boolean => {
...
export const extraerPartesIBAN = (iban: string): PartesIBAN => {
  const ibanLimpio = limpiarIBAN(iban);
  const ibanDesglosado = ibanLimpio.match(
    /^(ES)(\d{2})(\d{4})(\d{4})(\d{2})(\d{10})$/
  );

  if (!ibanDesglosado) {
    throw new Error("El IBAN no cumple con el formato esperado");
  }

  const [, pais, digitoControl, banco, oficina, dcCuenta, cuenta] =
    ibanDesglosado;
...
export const validarIBAN = (iban: string): void => {
  if (!esFormatoIBANValido(iban)) {
    throw new Error("Formato IBAN inválido");
  }
...
export const procesarIBAN = (
  iban: string
): { partes: PartesIBAN; banco: string } => {
  const partes = extraerPartesIBAN(iban);
  const banco = obtenerNombreBanco(partes.banco);
  return { partes, banco };
};
